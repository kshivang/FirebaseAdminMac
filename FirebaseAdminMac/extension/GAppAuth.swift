...
public final class GAppAuth: NSObject {
    ...
    private var errorPS = PassthroughSubject<(OIDAuthState, Error), Never>()
    public var errorPublisher: AnyPublisher<(OIDAuthState, Error), Never> {
        errorPS.eraseToAnyPublisher()
    }

    // Authorization changed, subscribe if you're interested
    private var stateChangePS = PassthroughSubject<OIDAuthState, Never>()
    public var stateChangePublisher: AnyPublisher<OIDAuthState, Never> {
        stateChangePS.eraseToAnyPublisher()
    }
